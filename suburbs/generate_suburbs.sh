#!/bin/bash
TEMPLATE="suburbs/suburb-template.html"

# Official List of 56 Suburbs
SUBURBS=(
    "Melbourne" "Docklands" "Southbank" "East-Melbourne" "West-Melbourne"
    "Carlton" "Carlton-North" "Fitzroy" "Fitzroy-North" "Collingwood"
    "Brunswick" "Brunswick-East" "Brunswick-West" "Northcote" "Thornbury"
    "Preston" "Essendon" "Moonee-Ponds" "Kensington" "Flemington"
    "Coburg" "Coburg-North" "Pascoe-Vale" "Pascoe-Vale-South" "Reservoir"
    "South-Yarra" "Prahran" "Windsor" "St-Kilda" "St-Kilda-East"
    "Elwood" "Ripponlea" "Balaclava" "Albert-Park" "Hampton" "Middle-Park" "Brighton"
    "Richmond" "Burnley" "Hawthorn" "Hawthorn-East" "Kew" "Kew-East"
    "Abbotsford" "Camberwell" "Balwyn" "Balwyn-North" "Box-Hill"
    "Footscray" "Seddon" "Yarraville" "Ascot-Vale" "Maribyrnong"
    "Keilor-East" "Sunshine" "Altona" "Maidstone"
)
...
for i in "${!SUBURBS[@]}"; do
    ...
    case "$suburb" in
        "Melbourne")
            HERO="Elite apartment cleaning for Melbourne CBD residents. Professional standards for the heart of the city."
            WHY="Living in the city center requires efficiency. We provide a reliable house cleaning service that fits perfectly with your urban lifestyle in {{SUBURB}}."
            IMAGE="https://images.unsplash.com/photo-1470075801209-17f9ec0cada6?auto=format&fit=crop&q=80&w=1470"
            ;;
        "Docklands")
        ...
        *)
            ...
            # Assign a different image based on index to ensure variety
            IMAGE_INDEX=$((i % ${#FALLBACK_IMAGES[@]}))
            IMAGE="${FALLBACK_IMAGES[$IMAGE_INDEX]}"
            ;;
    esac
    
    echo "Processing suburbs/$FILENAME (Case: Sentence Case & No Hospitality Standard)..."
    
    H_DESC=$(echo "$HERO" | sed "s/{{SUBURB}}/$DISPLAY_NAME/g")
    W_DESC=$(echo "$WHY" | sed "s/{{SUBURB}}/$DISPLAY_NAME/g")
    SAFE_IMAGE=$(echo "$IMAGE" | sed 's/&/\\\&/g')

    sed "s/{{SUBURB}}/$DISPLAY_NAME/g; s/{{SUBURB_DESC}}/$H_DESC/g; s/{{WHY_CHOOSE_DESC}}/$W_DESC/g; s|{{SUBURB_IMAGE}}|$SAFE_IMAGE|g" "$TEMPLATE" > "suburbs/$FILENAME"
done
